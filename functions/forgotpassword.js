...
exports.handler = function (event, context, callback) {
    // Request a la API
    const URL = "https://apiv4.ordering.co/v400/en/farmazone/users/forgot";
    ...
    const send = (statusCode, body) => {
        ...
        if (statusCode == 200) {
            res = cleanJSON(body);
            // res = JSON.stringify({ result: "Exito al enviar correo" })
        } else {
            res = JSON.stringify({ result: "Fallo al enviar correo." })
        }
        callback(null, {
            statusCode,
            headers: {
